...
namespace Day_4___High_Entropy_Passphrases
{
    class Program
    {
...
"lpyubo dylpfb iehwug decj ntidy cuygyg lalkb iutu oxgm imn                        ",
            ...
            {
                var punctuation = entry.Where(Char.IsPunctuation).Distinct().ToArray();
                var words = entry.Split().Select(x => x.Trim(punctuation)).ToList();
                var isValid = true;

                for (int i = 0; i < words.Count; i++)
                {
                    for (int y = i + 1; y < words.Count; y++)
                    {
                        if (words[i].Equals(words[y]) && !string.IsNullOrWhiteSpace(words[i]))
                        {
                            isValid = false;
                        }
                    }
            ...
            Console.WriteLine("High-Entropy Passphrase part 1: " + result);
            ...
            foreach (var entry in data)
            {
                ...
                for (int i = 0; i < words.Count; i++)
                {
                    for (int y = i + 1; y < words.Count; y++)
                    {
                        ...
                        var secondWord = String.Concat(words[y].OrderBy(c => c));

                        if (firstWord.Equals(secondWord) && !string.IsNullOrWhiteSpace(firstWord))
                        {
                            isValid = false;
                        }
                    }
            ...
            Console.WriteLine("High-Entropy Passphrase part 2: " + result);
            Console.ReadLine();
        }
    }
}
